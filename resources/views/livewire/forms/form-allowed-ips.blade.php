    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        @foreach($ips as $index => $ip)
            <x-card>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <x-input-control label="IP {{ $index + 1 }}" value="{{ $ip['ip'] }}" wire:model="ips.{{ $index }}.ip" maxlength="15"
                            pattern="^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|$)){4}$" required />
                    </div>

                    <div>
                        <x-input-control type="date" label="Fecha límite" value="{{ $ip['expires_at'] }}"
                            wire:model="ips.{{ $index }}.expires_at" />
                    </div>

                </div>
                <div>
                    <x-textarea-control label="Anotación" wire:model="ips.{{ $index }}.notes" rows="4"
